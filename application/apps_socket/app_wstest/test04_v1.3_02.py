...
# EXTENSIONES_MONITOREADAS = ['1000', '1001', '1002', 's', '6400', 'simulta', '6500']
EXTENSIONES_MONITOREADAS = ['1000', '1001']
...
def identificar_estado_canal(event: dict) -> str:
    """
    Clasifica el estado del canal en el evento: Llamada establecida (en curso).
    """
    tipo_evento = event.get("ChannelStateDesc")
    if tipo_evento == "Up":
        return True
    return False
...
def numero_involucrada(event: dict) -> str:
    """
    Intenta obtener número externo monitoreada involucrada en el evento.
    """
    for key in ["ConnectedLineNum"]:
        numero = event.get(key, "")
        if len(numero) > 4 and numero not in EXTENSIONES_MONITOREADAS:
            return numero
    return None
        ...
        while True:
            ...
            if event_data.strip():
                ...
                if "Event" in event_dict:
                    ...
                    estado_canal_up = identificar_estado_canal(event_dict)
                    numero = numero_involucrada(event_dict)

                    if ext and tipo and estado_canal_up and numero:
                        tiempo = datetime.now().strftime("%Y%m%d-%H%M%S.%f")
                        event_dict.update({"time":tiempo})
                        json_obj = {
                            "tipo": tipo,
                            "extension": ext,
                            "evento": event_dict
