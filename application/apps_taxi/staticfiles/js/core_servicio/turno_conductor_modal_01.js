var fnTblEscogerTurno01 = null;
var varTblEscogerTurno01_lengthPage = 0;
var fnTblEscTurnoUncheck = null;
var varTblEscogerTurno01_turnoAutomatico = 0;
$(document).ready(function () {
  const $tblEscogerTurno01 = $("#lista-turnos-escoger-01");
  const arrayOcultarColumaEscogerTurno01Datatable = [0];

  const TURNO_NO_CHECKET = `<i class="fa fa-square"></i>`;
  const TURNO_CHECKET = `<i class="fa fa-check"></i>`;

  function ocultarColumEscogerTurno01Datatable() {
    const filas = $("#lista-turnos-escoger-01 tbody").find("tr").length;
    if (arrayOcultarColumaEscogerTurno01Datatable.length === 0) {
      $("#lista-turnos-escoger-01 thead tr").find("th").eq(0).hide();
      for (x = 0; x < filas; x++) {
        $("#lista-turnos-escoger-01 tbody tr").eq(x).find("td").eq(0).hide();
      }
    } else {
      arrayOcultarColumaEscogerTurno01Datatable.map((item) =>
        $("#lista-turnos-escoger-01 thead tr").find("th").eq(item).hide()
      );
      for (x = 0; x < filas; x++) {
        arrayOcultarColumaEscogerTurno01Datatable.map((item) => $("#lista-turnos-escoger-01 tbody tr").eq(x).find("td").eq(item).hide()
        );
      }
    }
  }

  const tblEscogerTurno01 = $tblEscogerTurno01.DataTable({
    ...
    ajax: {
      ...
      data: function (params) {
        const choicesRowXPag_col02 = "#lista-turnos-escoger-02_length select[name=lista-turnos-escoger-02_length]";
        params["view_column"] = "01";
        if (varTblEscogerTurno01_lengthPage != parseInt(params.length)) {
          varTblEscogerTurno01_lengthPage = parseInt(params.length);
          $($(choicesRowXPag_col02)[0]).val(varTblEscogerTurno01_lengthPage);
          $($(choicesRowXPag_col02)[0]).prop("disabled", true);
          fnTblEscogerTurno02SetPageLength(varTblEscogerTurno01_lengthPage);
        }
        // $("#lista-turnos-escoger-02_length").hide();
        if (varTblEscogerTurno01_turnoAutomatico === 1) {
          params["turno_automatico"] = varTblEscogerTurno01_turnoAutomatico;
      ...
      dataSrc: function (jsonData) {
        ...
        if (recordReal > 0 && recordReal > varTblEscogerTurno01_lengthPage)
          setTimeout(function () {
            fnTblEscogerTurno02PageInitial();
          }, 300);
        return _data;
      },
      error: function () {
    ...
    columns: [
      ...
      },
      {
        ...
        className: "text-left font-12 font-weight-bold",
        searchable: false,
        orderable: false
      },
      {
    ...
    drawCallback: function (settings) {
      ocultarColumEscogerTurno01Datatable();
    }
  });

  $("#lista-turnos-escoger-01 tbody").on("click", ".details-control", function () {
    var tr = $(this).closest("tr"), row = tblEscogerTurno01.row(tr), datos = [];
    ...
    if (row.child.isShown()) {
      ...
      var dataCabecera = arrayOcultarColumaEscogerTurno01Datatable.map(
        (item) => `<th>${$("#lista-turnos-escoger-01 thead tr").find("th").eq(item).html()}</th>`
      );
      var dataDesc = arrayOcultarColumaEscogerTurno01Datatable.map((item) => `<td>${datos[item]}</td>`);
      var htmlInfo = `
  ...
  $("#lista-turnos-escoger-01 tbody").on("click", ".seleccionar-turno", function () {
  ...
  fnTblEscogerTurno01 = function () {
    try {
      tblEscogerTurno01.search("");
      tblEscogerTurno01.ajax.reload(null, true);
    } catch (e) {
      console.error(e);
    }
  };
