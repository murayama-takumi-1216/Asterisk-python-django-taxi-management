...
$(document).ready(function () {
  ...
  const arrayOcultarColumaTurnoConductorDatatable = [8, 10];
  ...
  const tblTurnoConductor = $tblTurnoConductor.DataTable({
    ...
    ajax: {
      cache: false,
      url: urlList.turnoConductor.listaConductores,
      method: "get",
      dataType: "json",
      data: function (params) {
        let codigoConductor = $("#id_turncond_codigo_conductor").val() ?? "";
        if (codigoConductor.length > 2) {
          params["filtro_codigo_conductor"] = codigoConductor;
        }
        let ApellidoPaternoConductor = $("#id_turncond_ape_paterno_conductor").val() ?? "";
        if (ApellidoPaternoConductor.length > 1) {
          params["filtro_apellido_paterno_conductor"] = ApellidoPaternoConductor;
        }
        ...
        let estadoTurno = $("#id_turncond_estado_turno").val() ?? "";
        if (estadoTurno.length > 1) {
          params["filtro_estado_turno"] = estadoTurno;
        }
        ...
        let codigoHorario = $("#id_turncond_codigo_horario").val() ?? "";
        if (codigoHorario.length > 1) {
          params["filtro_codigo_horario"] = codigoHorario;
        }
        return params;
      },
    ...
    columns: [
      ...
      },
      {
        data: function (data) {
          ...
          let btnFiltarDriver = `<button type="button" data-codigoconductor="${conductor.cod_conductor ?? ""}"
            class="btn btn-sm btn-secondary filtrar-driver"><i class="fa fa-search text-cyan"></i></button>`
          let datosConductor = `${conductor.nombre ?? ""} ${conductor.apellido_paterno ?? ""} ${btnFiltarDriver}`;
      ...
      },
      {
        data: function (data) {
          ...
          let datosVehiculo = `
            <span class="label label-megna pt-1 pb-1 font-16">${vehiculo.nombre}</span>`;
      ...
      },
      {
        ...
        width: "210",
        data: function (data) {
          // let atendidos = parseInt(data.servicios_asignados ?? "0") - parseInt(data.servicios_cancelados ?? "0");
          let serviciosAtendidos = parseInt(data.servicios_atendidos ?? "0");
          let serviciosCancelados = parseInt(data.servicios_cancelados ?? "0");
          // let htmlAsignados = `
          //   <span class="label label-megna mytooltip" title="Asignados">${data.servicios_asignados ?? "0"}
          //       <span class="tooltip-content-cap tooltip-content-cap-cyan"><span class="tooltip-text"><span
          //       class="tooltip-inner2">Asignados</span></span></span></span>`;
          let htmlCancelados = `
            <span class="label label-primary mytooltip" title="Asignados">${serviciosCancelados}
                <span class="tooltip-content-cap tooltip-content-cap-primary"><span class="tooltip-text"><span
                class="tooltip-inner2">Cancelados</span></span></span></span>`;
          let htmlAtendidos = `
            <span class="label label-success mytooltip mr-1" title="Asignados">${serviciosAtendidos}
                <span class="tooltip-content-cap tooltip-content-cap-success"><span class="tooltip-text"><span
                class="tooltip-inner2">Realizados</span></span></span></span>`;
          // let htmlAtendidosProcess = `
          //   <span class="label label-default text-black-50">${data.servicios_atendidos ?? "0"}</span>`;
          // let htmlServicios = `
          //   ${htmlAsignados}
          //   <i class="fa fa-minus text-cyan ml-1 mr-1"></i> ${htmlCancelados}
          //   <i class="fa fa-equals text-cyan ml-1 mr-1"></i> ${htmlAtendidos} | ${htmlAtendidosProcess}`;
          let htmlServicios = `${htmlAtendidos} ${htmlCancelados}`;
  ...
  $btnLimpiarFiltroTurnoConductoresList.click(function () {
    ...
    $("#id_turncond_ape_paterno_conductor").val("");
