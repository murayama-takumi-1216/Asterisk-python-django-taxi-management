  ...
  const tblTurnoConductor = $tblTurnoConductor.DataTable({
    ...
    ajax: {
      ...
      data: function (params) {
        let licencia = $("#id_turncond_licencia").val() ?? "";
        if (licencia.length > 2) {
          params["filtro_licencia"] = licencia;
        }
        ...
        let matricula = $("#id_turncond_matricula").val() ?? "";
        if (matricula.length > 1) {
          params["filtro_matricula"] = matricula;
        }
    ...
    columns: [
      ...
      },
      {
        data: function (data) {
          ...
          let datosConductor = `${conductor.apellido_paterno ?? ""} ${conductor.apellido_materno ?? ""},
            ${conductor.nombre ?? ""}`;
      ...
      {
        className: "text-center border-left font-weight-bold",
        searchable: false,
        orderable: false,
        data: function (data) {
      ...
      },
      {
        ...
        data: function (data) {
          let horario = data.horario_data ?? {};
          let hora_inicio = (horario.inicio ?? ".").substring(0, 5);
          return `<span class="text-black-50">${data.fecha_program_ini_view} ${hora_inicio}</span>`;
        }
      },
      {
        className: "text-center font-weight-bold",
        ...
        data: function (data) {
          let horario = data.horario_data ?? {};
          let hora_fin = (horario.fin ?? ".").substring(0, 5);
          return `<span class="text-black-50">${data.fecha_program_fin_view} ${hora_fin}</span>`;
        }
      }
    ],
